...
export default function SearchFilterLayout({
  ...
  return (
    <main className="group flex flex-col items-center justify-center gap-10 p-10 px-5 lg:flex-row lg:items-start">
      <aside
...
function SortFilter({ sort, updateSort }: SortFilterProps) {
  return (
    <Select value={sort || "last_updated"} onValueChange={updateSort}>
      ...
      <SelectContent>
        <SelectItem value="last_updated">Nejnovější</SelectItem>
        <SelectItem value="price_asc">Cena (nejnižší po nejvyšší)</SelectItem>
        <SelectItem value="price_desc">Cena (nejvyšší po nejnižší)</SelectItem>
      </SelectContent>
    </Select>
  );
}
